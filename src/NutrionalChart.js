...
const NutrionalChart = () => {
  const { protein, carbohydrates, fat, fiber, mineral, Vitamin, isLoading, fetchError } = useContext(DataContext);

  const handleTableRows = (nutrient) => {
    if (!nutrient) {
      return null; // or some default content or an empty array
    }
  ...
  // State to manage the visibility of lists
